...
from hetsgg.modeling.roi_heads.relation_head.utils_relation import get_box_pair_info, get_box_info, \
    layer_init
...
class PairwiseFeatureExtractor2(nn.Module):
    """
    ...
    def __init__(self, config, in_channels):
        super(PairwiseFeatureExtractor2, self).__init__()
        self.cfg = config
        ...
        self.entity_emb = nn.Embedding(151,self.hidden_dim)
        self.relation_emb = nn.Embedding(51,self.hidden_dim)
        self.aug_entity = make_fc(self.hidden_dim+self.embed_dim,self.hidden_dim)
        self.aug_relation = nn.Linear(self.hidden_dim * 2, self.pooling_dim)
        self.aug_obj = nn.Linear(self.hidden_dim * 2, self.pooling_dim)
    ...
    def forward(self, triplets):
        ...
        """
        ent_struct_emb = self.entity_emb.weight   
        ent_struct_word_emb = self.obj_embed_on_pred_label.weight #200
      

        # rel_struct_emb = self.relation_emb(triplets[:,1])

        ent_struct_emb = cat((ent_struct_emb,ent_struct_word_emb), dim=1)
        ent_struct_emb = self.aug_entity(ent_struct_emb)
        rel_struct_feat = cat((ent_struct_emb[triplets[:,0]],ent_struct_emb[triplets[:,2]]),dim=1)
        rel_struct_feat = self.aug_relation(rel_struct_feat)

        

        return ent_struct_emb, rel_struct_feat
