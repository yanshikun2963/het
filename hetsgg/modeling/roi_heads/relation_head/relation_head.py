...
from hetsgg.utils.visualize_graph import *
...
class ROIRelationHead(torch.nn.Module):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        roi_features = self.box_feature_extractor(features, proposals) # Box Feature
        if isinstance(self.box_feature_extractor, ResNet50Conv5ROIFeatureExtractor):
        ...
        if self.rel_prop_on: # True
            ...
            if self.rel_prop_type == "rel_pn":
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.HETSGG.USE_REL_PN and not self.training:
                    _, _, selected_rel_pairs_idx = filter_rel_pairs(relness_matrix, rel_pair_idxs, rel_labels)
                elif self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR == 'AGRCNNPredictor':
                    rel_pair_idxs, rel_labels , _ = filter_rel_pairs(relness_matrix, rel_pair_idxs, rel_labels)
        ...
        if not self.training:
            if self.cfg.MODEL.ROI_RELATION_HEAD.HETSGG.USE_REL_PN:
                rel_pair_idxs = [rel_pair[selected_idx] for rel_pair, selected_idx in zip(rel_pair_idxs, selected_rel_pairs_idx)]
                relation_logits = [relation_logit[select_idx] for relation_logit, select_idx in zip(relation_logits, selected_rel_pairs_idx)]
            if not self.object_cls_refine: # False
