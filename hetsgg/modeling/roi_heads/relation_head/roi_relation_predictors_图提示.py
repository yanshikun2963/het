...
class PrototypeEmbeddingNetwork(nn.Module):
    def __init__(self, config, in_channels):
        ...
        proto = '/home/yj/zgw/het/proto/muil_sgl_p.h5'
        ...
        ### for prompt
        pt_path = '/home/yj/zgw/het/p_data/vg/train'
        ...
        self.rel_emb_forward, self.rel_emb_inverse, _ = self.prompt_vec2(torch.arange(51))
        self.prompt_forward = nn.Embedding(51,32)
        self.prompt_inverse = nn.Embedding(51,32)
        t1, t2 = torch.zeros(51,32) , torch.zeros(51,32)
        for i in range(51):
            t1[i,:], t2[i,:] = self.rel_emb_forward[i,i,:], self.rel_emb_inverse[i,i,:]

        with torch.no_grad():
            self.prompt_forward.weight.copy_(t1, non_blocking=True)
            self.prompt_inverse.weight.copy_(t2, non_blocking=True)

        self.prompt_W_forward = MLP(32, self.mlp_dim * 2, self.mlp_dim, 2)
        self.norm_prompt_rep_forward = nn.LayerNorm(self.mlp_dim)
        self.dropout_prompt_rep_forward = nn.Dropout(dropout_p)
        self.linear_prompt_rep_forward = nn.Linear(self.mlp_dim, self.mlp_dim)
        self.dropout_prompt_forward = nn.Dropout(dropout_p)
        self.project_head_prompt_forward = MLP(self.mlp_dim, self.mlp_dim, self.mlp_dim*2, 2)
        self.dropout_pred_prompt_forward = nn.Dropout(dropout_p)
        self.logit_scale_prompt_forward = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))

        self.prompt_W_inverse = MLP(32, self.mlp_dim * 2, self.mlp_dim, 2)
        self.norm_prompt_rep_inverse = nn.LayerNorm(self.mlp_dim)
        self.dropout_prompt_rep_inverse = nn.Dropout(dropout_p)
        self.linear_prompt_rep_inverse = nn.Linear(self.mlp_dim, self.mlp_dim)
        self.dropout_prompt_inverse = nn.Dropout(dropout_p)
        self.project_head_prompt_inverse = MLP(self.mlp_dim, self.mlp_dim, self.mlp_dim*2, 2)
        self.dropout_pred_prompt_inverse = nn.Dropout(dropout_p)
        self.logit_scale_prompt_inverse = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))

        layer_init(self.linear_prompt_rep_inverse,xavier=True)
        layer_init(self.linear_prompt_rep_forward,xavier=True)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None, is_training=True):
        ...
        # n_ent = self.data_loader[0].kg.entity_num
        ...
        rel_dists_sem = rel_rep_norm @ predicate_proto_norm.t() * self.logit_scale.exp()  #  <r_norm, c_norm> / τ

        # _, rel_list = torch.topk(rel_dists_sem,k=2,dim=1)
        # r_list = np.zeros(rel_list.shape[0])
        # for i in range(rel_list.shape[0]):
        #     if rel_list[i,0] != 0:
        #         r_list[i] = rel_list[i,0]
        #     else:
        #         r_list[i] = rel_list[i,1]

        # r_list = torch.from_numpy(r_list)
        ...
        # #vis_prototpye_0  use het rel_rep + prompt
        ...
        #     vis_rep_x = fusion_prompt
        #     vis_rep_x = self.prompt_linear(vis_rep_x)
            
        #     vis_proto_p = self.vis_W_p(self.vis_embed_p.weight)

        #     #vis model convergence
        #     vis_rep_x = self.norm_vis_rep_p(self.dropout_vis_rep_p(torch.relu(self.linear_vis_rep_p(vis_rep_x))) + vis_rep_x)
        #     vis_rep_x = self.project_head_vis_p(self.dropout_vis_p(torch.relu(vis_rep_x)))
        #     predicate_proto_vis_p = self.project_head_vis_p(self.dropout_pred_vis_p(torch.relu(vis_proto_p)))

        #     vis_rep_norm_x = vis_rep_x / vis_rep_x.norm(dim=1, keepdim=True)
        #     predicate_proto_vis_norm_p = predicate_proto_vis_p / predicate_proto_vis_p.norm(dim=1, keepdim=True)

        #     vis_dist_p = vis_rep_norm_x @ predicate_proto_vis_norm_p.t() * self.logit_scale_vis_p.exp()
        

        # end
        # group1, group2, group3 = self.random_rel(fusion_so)
        #vis_prototype_0
        prompt_rep_x = rel_feat
        
        
        prompt_proto_forward = self.prompt_W_forward(self.prompt_forward.weight)

        #vis model convergence
        prompt_rep_forward = self.norm_prompt_rep_forward(self.dropout_prompt_rep_forward(torch.relu(self.linear_prompt_rep_forward(prompt_rep_x))) + prompt_rep_x)
        prompt_rep_forward = self.project_head_prompt_forward(self.dropout_prompt_forward(torch.relu(prompt_rep_forward)))
        predicate_proto_prompt_forward = self.project_head_prompt_forward(self.dropout_pred_prompt_forward(torch.relu(prompt_proto_forward)))

        prompt_rep_norm_forward = prompt_rep_forward / prompt_rep_forward.norm(dim=1, keepdim=True)
        predicate_proto_vis_norm_0 = predicate_proto_prompt_forward / predicate_proto_prompt_forward.norm(dim=1, keepdim=True)

        prompt_dist_forward = prompt_rep_norm_forward @ predicate_proto_vis_norm_0.t() * self.logit_scale_prompt_forward.exp()



        # prompt_rep_x = fusion_so
        
        
        prompt_proto_inverse = self.prompt_W_inverse(self.prompt_inverse.weight)

        #vis model convergence
        prompt_rep_inverse = self.norm_prompt_rep_inverse(self.dropout_prompt_rep_inverse(torch.relu(self.linear_prompt_rep_inverse(prompt_rep_x))) + prompt_rep_x)
        prompt_rep_inverse = self.project_head_prompt_inverse(self.dropout_prompt_inverse(torch.relu(prompt_rep_inverse)))
        predicate_proto_prompt_inverse = self.project_head_prompt_inverse(self.dropout_pred_prompt_inverse(torch.relu(prompt_proto_inverse)))

        prompt_rep_norm_inverse = prompt_rep_inverse / prompt_rep_inverse.norm(dim=1, keepdim=True)
        predicate_proto_vis_norm_0 = predicate_proto_prompt_inverse / predicate_proto_prompt_inverse.norm(dim=1, keepdim=True)

        prompt_dist_inverse = prompt_rep_norm_inverse @ predicate_proto_vis_norm_0.t() * self.logit_scale_prompt_forward.exp()

        #end

        prompt_dist = (prompt_dist_forward + prompt_dist_inverse) / 2
        ...
        #vis model convergence
        vis_rep_2 = self.norm_vis_rep_2(self.dropout_vis_rep_2(torch.relu(self.linear_vis_rep_2(vis_rep_))) + vis_rep_)
        ...
        # end
        # if len(unique_rels) > 0:
        #     avg_vis_proto_dist = (vis_dist_0 + vis_dist_1 + vis_dist_2 + vis_dist_p * 0.0001) / 3
        # else:
        #     avg_vis_proto_dist = (vis_dist_0 + vis_dist_1 + vis_dist_2) / 3

        
        # the rel_dists will be used to calculate the Le_sim with the ce_loss
        avg_vis_proto_dist = (vis_dist_1 + vis_dist_2 + prompt_dist) / 3

        rel_dists = rel_dists_sem + avg_vis_proto_dist
        ...
        if self.training:
            ...
            # ### end
            prompt_dist_loss_forward = self.get_guss_loss(predicate_proto_prompt_forward)
            prompt_dist_loss_inverse = self.get_guss_loss(predicate_proto_prompt_inverse)

            add_losses.update({"prompt_dist_loss_forward": prompt_dist_loss_forward * 0.2})
            add_losses.update({"prompt_dist_loss_inverse": prompt_dist_loss_inverse * 0.2})
            ...
            rep_proto_loss_prompt_forward = self.get_rep_pro_loss(prompt_rep_forward,predicate_proto_prompt_forward,rel_labels)
            rep_proto_loss_prompt_inverse = self.get_rep_pro_loss(prompt_rep_inverse,predicate_proto_prompt_inverse,rel_labels)
            add_losses.update({"rep_proto_loss_prompt_forward": rep_proto_loss_prompt_forward * 0.2})
            add_losses.update({"rep_proto_loss_prompt_inverse": rep_proto_loss_prompt_inverse * 0.2})
    ...
    def prompt_vec2(self, unique_rel):
        # unique_rels = unique_triplet[:,1].cpu().numpy()
        unique_rels, unique_indices = np.unique(unique_rel.cpu().numpy(), return_inverse=True)
        
        repeat_rels = np.repeat(np.expand_dims(unique_rels, 1), self.cfg.MODEL.PROMPTENCODER.SHOT, axis=1).reshape(-1) #把唯一关系重复5次  5x

        edge_index, edge_type, h_positions, t_positions, query_relations, edge_query_relations, labels, num_ent, is_None = self.data_loader[0].get_case_graph(repeat_rels #导入整个数据集的图，x_position是query头尾节点的在提取出的子图中的索引
                                                                                                                     )    
        if is_None == False:        
            rel_embeddings, query_embeddings, rel_embedding_full = self.relation_encoder(edge_index, edge_type, h_positions, t_positions, query_relations, edge_query_relations, labels,
                                                    num_ent, self.data_loader[0], shot=self.cfg.MODEL.PROMPTENCODER.SHOT)
            ...
            # m = self.for_zero_rel_forward.weight
            # m = m.unsqueeze(0)
            # rel_emb_forward = cat((m.cuda(),rel_emb_forward.cuda()), dim=0).cuda()
            # rel_emb_inverse = cat((m.cuda(),rel_emb_inverse.cuda()), dim=0).cuda()
    ...
    def prompt_vec(self, unique_triplet):
        ...
        if is_None == False:        
            ...
            m = self.for_zero_rel_forward.weight
            m = m.unsqueeze(0)
            rel_emb_forward = cat((m,rel_emb_forward), dim=0).cuda()
            rel_emb_inverse = cat((m,rel_emb_inverse), dim=0).cuda()
    ...
    def get_triplet(self, proposals, rel_pair_idx, rel_labels):
        ...
        edge_idx = edge_idx.unsqueeze(dim=0).t().cuda()
        triplet_head = rel[:,:1].cuda()
        triplet_tail = rel[:,1:].cuda()
        ...
        for i in range(1,len(proposals)):
            ...
            edge_idx_ = edge_idx_.unsqueeze(dim=0).t().cuda()
            triplet_head_ = rel_p[:,:1].cuda()
            triplet_tail_ = rel_p[:,1:].cuda()
    ...
    def get_triplet2(self, proposals, rel_pair_idx, rel_labels):
        rel = self.find_label(proposals[0].get_field("labels"),rel_pair_idx[0])
        edge_idx = rel_labels[:rel_pair_idx[0].shape[0]]
        no_zero_indice = torch.nonzero(edge_idx).squeeze(dim=1)
        edge_idx = edge_idx.unsqueeze(dim=0).t().cuda()
        triplet_head = rel[:,:1].cuda()
        triplet_tail = rel[:,1:].cuda()
        ...
        if len(rel_pair_idx) > 1:
            for i in range(1,len(proposals)):
                rel_p = self.find_label(proposals[i].get_field("labels"), rel_pair_idx[i])
                edge_idx_ = rel_labels[first:(first + rel_pair_idx[i].shape[0])]
                no_zero_indice_ = torch.nonzero(edge_idx_).squeeze(dim=1)
                edge_idx_ = edge_idx_.unsqueeze(dim=0).t().cuda()
                triplet_head_ = rel_p[:,:1].cuda()
                triplet_tail_ = rel_p[:,1:].cuda()
                ...
                triplet = cat((triplet,triplet_),dim=0)
                first = first + rel_pair_idx[i].shape[0]
