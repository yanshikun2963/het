...
class PromptEncoder(torch.nn.Module):
    def __init__(self, config):
        ...
        self.start_relation_embeddings.cuda()
        ...
        self.position_embedding.cuda()
        self.self_loop_embedding = nn.Embedding(1, self.hidden_dim)
        self.self_loop_embedding.cuda()
        ...
        self.W_ht2r = nn.ModuleList([nn.Linear(self.hidden_dim * 3, self.hidden_dim, bias=True).cuda() for _ in range(self.n_layer)])
        self.W_message = nn.ModuleList([nn.Linear(self.hidden_dim * 3 if self.cfg.MODEL.PROMPTENCODER.MSG == 'concat' else self.hidden_dim * 5, self.hidden_dim, bias=True).cuda() for _ in range(self.n_layer)])
        self.alpha = nn.ModuleList([nn.Linear(self.hidden_dim * 2, 1, bias=True).cuda() for _ in range(self.n_layer)])
        self.beta = nn.ModuleList([nn.Linear(self.hidden_dim * 2, 1, bias=True).cuda() for _ in range(self.n_layer)])
        self.loop_transfer = nn.ModuleList([nn.Linear(self.hidden_dim * 2, self.hidden_dim, bias=True).cuda() for _ in range(self.n_layer)])
        self.ent_transfer = nn.ModuleList([nn.Linear(self.hidden_dim, self.hidden_dim, bias=True).cuda() for _ in range(self.n_layer)])
        self.rel_transfer = nn.ModuleList([nn.Linear(self.hidden_dim, self.hidden_dim, bias=True).cuda() for _ in range(self.n_layer)])

        # readout
        self.final_to_rel_embeddings = nn.Linear(self.hidden_dim * self.n_layer, self.hidden_dim).cuda()

        # dropout and layer normalization
        self.dropout = nn.Dropout(0.3)
        self.layer_norm_rels = nn.ModuleList([nn.LayerNorm(self.hidden_dim).cuda() for _ in range(self.n_layer+1)])
        self.layer_norm_ents = nn.ModuleList([nn.LayerNorm(self.hidden_dim).cuda() for _ in range(self.n_layer+1)])
        self.layer_norm_loop = nn.ModuleList([nn.LayerNorm(self.hidden_dim).cuda() for _ in range(self.n_layer+1)])
    ...
    def forward(self, edge_index, edge_type, h_positions, t_positions, query_relations, edge_query_relations, labels, num_ent, loader, shot=5):
        ...
        if self.cfg.MODEL.PROMPTENCODER.USE_TOKEN_SET: #True
            ...
            node_embeddings = torch.zeros(num_ent, self.hidden_dim)
            rel_embeddings = torch.zeros(relation_num * h_positions.size(-1), self.hidden_dim)
