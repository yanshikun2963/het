...
class PrototypeEmbeddingNetwork(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None, is_training=True):
        ...
        # obj_feats, rel_feats = self.context_layer(roi_features, union_features, proposals, rel_pair_idxs, rel_binarys, logger, is_training) # GNN
        ...
        # rel_cls_logits = self.rel_classifier(rel_feats)  # input_dim:51  output_dim:51
        ...
        if self.training:
            ...
            rel_labels_vis_train_0 = rel_labels_vis_train[:group_size]
            rel_labels_vis_train_1 = rel_labels_vis_train[group_size:2*group_size]
            rel_labels_vis_train_2 = rel_labels_vis_train[2*group_size:2*group_size+last_group_size]
            ### Prototype-based Learning for Vis (0)---- Euclidean distance
            # gamma1_0 = 1.0
            # vis_rep_expand_0 = vis_rep_0.unsqueeze(dim=1).expand(-1,51,-1)
            # predicate_proto_expand_0 = predicate_proto_vis_0.unsqueeze(dim=0).expand(rel_labels_vis_train_0.size(0),-1,-1)
            # distance_set_0 = (vis_rep_expand_0 - predicate_proto_expand_0).norm(dim=2) ** 2
            # mask_neg_0 = torch.ones(rel_labels_vis_train_0.size(0), 51).cuda()
            # mask_neg_0[torch.arange(rel_labels_vis_train_0.size(0)),rel_labels_vis_train_0] = 0
            # distance_set_neg_0 = distance_set_0 * mask_neg_0
            # distance_set_pos_0 = distance_set_0[torch.arange(rel_labels_vis_train_0.size(0)),rel_labels_vis_train_0]

            # sorted_distance_set_neg_0, _ = torch.sort(distance_set_neg_0,dim=1)
            # topK_sorted_distance_set_neg_0 = sorted_distance_set_neg_0[:, :11].sum(dim=1) / 10
            # loss_sum_0 = torch.max(torch.zeros(rel_labels_vis_train_0.size(0)).cuda(), distance_set_pos_0 - topK_sorted_distance_set_neg_0 + gamma1_0).mean()
            # add_losses.update({"loss_dis_vis_0": loss_sum_0 * 0.2})
            ...
            # gamma1_1 = 1.0
            # vis_rep_expand_1 = vis_rep_1.unsqueeze(dim=1).expand(-1,51,-1)
            # predicate_proto_expand_1 = predicate_proto_vis_1.unsqueeze(dim=0).expand(rel_labels_vis_train_1.size(0),-1,-1)
            # distance_set_1 = (vis_rep_expand_1 - predicate_proto_expand_1).norm(dim=2) ** 2
            # mask_neg_1 = torch.ones(rel_labels_vis_train_1.size(0), 51).cuda()
            # mask_neg_1[torch.arange(rel_labels_vis_train_1.size(0)),rel_labels_vis_train_1] = 0
            # distance_set_neg_1 = distance_set_1 * mask_neg_1
            # distance_set_pos_1 = distance_set_1[torch.arange(rel_labels_vis_train_1.size(0)),rel_labels_vis_train_1]

            # sorted_distance_set_neg_1, _ = torch.sort(distance_set_neg_1,dim=1)
            # topK_sorted_distance_set_neg_1 = sorted_distance_set_neg_1[:, :11].sum(dim=1) / 10
            # loss_sum_1 = torch.max(torch.zeros(rel_labels_vis_train_1.size(0)).cuda(), distance_set_pos_1 - topK_sorted_distance_set_neg_1 + gamma1_1).mean()
            # add_losses.update({"loss_dis_vis_1": loss_sum_1 * 0.2})
            ...
            # gamma1_2 = 1.0
            # vis_rep_expand_2 = vis_rep_2.unsqueeze(dim=1).expand(-1,51,-1)
            # predicate_proto_expand_2 = predicate_proto_vis_2.unsqueeze(dim=0).expand(rel_labels_vis_train_2.size(0),-1,-1)
            # distance_set_2 = (vis_rep_expand_2 - predicate_proto_expand_2).norm(dim=2) ** 2
            # mask_neg_2 = torch.ones(rel_labels_vis_train_2.size(0), 51).cuda()
            # mask_neg_2[torch.arange(rel_labels_vis_train_2.size(0)),rel_labels_vis_train_2] = 0
            # distance_set_neg_2 = distance_set_2 * mask_neg_2
            # distance_set_pos_2 = distance_set_2[torch.arange(rel_labels_vis_train_2.size(0)),rel_labels_vis_train_2]

            # sorted_distance_set_neg_2, _ = torch.sort(distance_set_neg_2,dim=1)
            # topK_sorted_distance_set_neg_2 = sorted_distance_set_neg_2[:, :11].sum(dim=1) / 10
            # loss_sum_2 = torch.max(torch.zeros(rel_labels_vis_train_2.size(0)).cuda(), distance_set_pos_2 - topK_sorted_distance_set_neg_2 + gamma1_2).mean()
            # add_losses.update({"loss_dis_vis_2": loss_sum_2 * 0.2})
