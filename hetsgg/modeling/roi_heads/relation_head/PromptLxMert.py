...
class GlobalPromptLxmert(nn.Module):
    ...
    def forward(self, input_ids, visual_feats, visual_pos, token_type_ids=None, attention_mask=None):
        ...
        language_last_hidden_state = outputs.language_hidden_states[-1]
        # # 确定 sub 和 obj 的位置
        # a_pos, b_pos = self._get_token_positions(input_ids)
        
        # # 提取嵌入
        sub_embeddings = language_last_hidden_state[:, 4, :]  # [batch_size, hidden_size]
        obj_embeddings = language_last_hidden_state[:, 7, :]  # [batch_size, hidden_size]
        rel_embeddings = language_last_hidden_state[:, 6, :]
