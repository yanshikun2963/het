...
class PrototypeEmbeddingNetwork(nn.Module):
    def __init__(self, config, in_channels):
        ...
        # self.vis_W_s = MLP(self.mlp_dim, self.mlp_dim * 2, self.mlp_dim, 2)
        # self.norm_vis_rep_s = nn.LayerNorm(self.mlp_dim)
        # self.dropout_vis_rep_s = nn.Dropout(dropout_p)
        # self.linear_vis_rep_s = nn.Linear(self.mlp_dim, self.mlp_dim)
        # self.dropout_vis_s = nn.Dropout(dropout_p)
        # self.project_head_vis_s = MLP(self.mlp_dim, self.mlp_dim, self.mlp_dim*2, 2)
        # self.dropout_pred_vis_s = nn.Dropout(dropout_p)
        # self.logit_scale_vis_s = nn.Parameter(torch.ones([]) * np.log(1 / 0.07))
        # self.struct_embed = nn.Embedding(51,2048)
        # with torch.no_grad():
        #     self.struct_embed.weight.copy_(self.vis_proto_1, non_blocking=True)
        ...
        self.MoERouter = UniformMoERouter3_()
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None, is_training=True):
        ...
        for proposal, rel_pair_idx,sub_rep,obj_rep in zip(proposals,rel_pair_idxs,sub_reps,obj_reps):
            ...
            # bert_outputs = self.Lxmermodel(
            #                         input_ids = bert_inputs["input_ids"].cuda(),
            ...
            # bert_outputs = self.Lx(input_ids =bert_inputs["input_ids"].cuda(),
            #                        attention_mask=bert_inputs["attention_mask"].cuda(),
            #                        visual_feats=visual_feat_s_o,
            #                        visual_pos=s_o_box)
            
            # mask_positions = torch.where(bert_inputs["input_ids"] == self.tokenizer.mask_token_id)[1] + 3
            # batch_logits = bert_outputs.prediction_logits[torch.arange(len(prompts)), mask_positions]
            # batch_logits[:, ~torch.isin(torch.arange(batch_logits.shape[-1]), torch.tensor(self.valid_predicate_idx))] = -float('inf')

            # # mask_logits = torch.stack([batch_logits[i, pos, :] for i, pos in enumerate(mask_positions)])
            # predicted_ids = torch.argmax(batch_logits, dim=-1)
            # word = self.tokenizer.batch_decode(predicted_ids, skip_special_tokens=True)
            # embedding_layer = self.Lxmermodel.lxmert.embeddings.word_embeddings
            # predicate_tensor = embedding_layer(predicted_ids)


            # last_hidden_state = bert_outputs.lxmert_output.last_hidden_state
            # sub_emb = last_hidden_state[:, 4, :]
            # obj_emb = last_hidden_state[:, 6, :]
        ...
        graph_prompt_indice_list = cat(graph_prompt_indice_list, dim=0) 
        ...
        pair_preds = []
        prod_reps = []
        for pair_idx, sub_rep, obj_rep, entity_pred, entity_embed, proposal, sub_emb, obj_emb,sub_vis,obj_vis in zip(rel_pair_idxs, sub_reps, obj_reps, entity_preds, entity_embeds, proposals,
                                                                                                      sub_emb_list,obj_emb_list,sub_vis_list,obj_vis_list):
        ...
        fusion_so = fusion_so + self.W_lxmert(rel_bert_list)
        ...
        expert_outputs, router_loss, indices = self.MoERouter(x=rel_moe,y=self.graph_prompt[graph_prompt_indice_list,:])
        # group1, group2, group3, group4 = expert_outputs[0], expert_outputs[1], expert_outputs[2], expert_outputs[3]
        # shuffle_idx = cat([indices[0],indices[1],indices[2],indices[3]])
        ...
        # rel_dists = rel_dists + vis_dist


        # rel_logits = F.softmax(rel_dists, dim=1)
        # indice_1 = torch.argmax(rel_logits, dim=1)

        # prompt_struct = self.struct_embed.weight[indice_1,:]
        # vis_proto_s = self.vis_W_s(self.struct_embed.weight)

        # vis_rep_s = (fusion_so - sem_pred * gate_sem_pred) + prompt_struct
        # vis_rep_s = self.r2s_lin(vis_rep_s)

        # #vis model convergence
        # vis_rep_s = self.norm_vis_rep_s(self.dropout_vis_rep_s(torch.relu(self.linear_vis_rep_s(vis_rep_s))) + vis_rep_s)
        # vis_rep_s = self.project_head_vis_s(self.dropout_vis_s(torch.relu(vis_rep_s)))
        # predicate_proto_vis_s = self.project_head_vis_s(self.dropout_pred_vis_s(torch.relu(vis_proto_s)))

        # vis_rep_norm_s = vis_rep_s / vis_rep_s.norm(dim=1, keepdim=True)
        # predicate_proto_vis_norm_s = predicate_proto_vis_s / predicate_proto_vis_s.norm(dim=1, keepdim=True)

        # vis_dist_s = vis_rep_norm_s @ predicate_proto_vis_norm_s.t() * self.logit_scale_vis_s.exp()

        # rel_dists = rel_dists + vis_dist_s * 0.15

        
        # group1, group2, group3, shuffle_idx, group_size, last_group_size = self.random_rel(rel_feat)
        # group1, group2, group3, shuffle_idx, group_size, last_group_size = self.random_rel(fusion_so - sem_pred * gate_sem_pred)  # 有效38.77

        # group1, group2, group3, shuffle_idx, group1_size,group2_size,_ = self.random_rel2(rel_feat,0.5,0.25,0.25)
        ...
        if self.training:
            ...
            # self.iter += 1 
            # ctx_gate_2 = self.post_cat_2(prod_rep)  # 1024 -> 4096
            # visual_rep_2 = ctx_gate_2 * union_features  
            # rel_dists_2 = self.rel_compress_2(visual_rep_2) + self.ctx_compress_2(prod_rep)
            # add_data.update({"rel_dists_2": rel_dists_2})
            ...
            ### end
            # l21_struct = self.get_l2loss(predicate_proto_vis_norm_s)
            # add_losses.update({"l21_struct":l21_struct* 0.2})
            ...
            ### end
            # dist_loss_struct = self.get_guss_loss(vis_proto_s)
            # add_losses.update({"dist_loss_struct": dist_loss_struct* 0.2 })
            ...
            # rel_labels_vis_train_2 = rel_labels_vis_train[group1.shape[0]+group2.shape[0]:group1.shape[0]+group2.shape[0]+group3.shape[0]]
            # rel_labels_vis_train_3 = rel_labels_vis_train[group1.shape[0]+group2.shape[0]+group3.shape[0]:]

            rel_labels_vis_train_0 = rel_labels_vis_train[:group1.shape[0]]
            rel_labels_vis_train_1 = rel_labels_vis_train[group1.shape[0]:group1.shape[0]+group2.shape[0]]
            rel_labels_vis_train_2 = rel_labels_vis_train[group1.shape[0]+group2.shape[0]:]

            # loss_dis = self.get_rep_pro_loss(rel_rep,predicate_proto,rel_labels_vis_train_0)
            # add_losses.update({"loss_dis": loss_dis * 0.8 })
            ...
            ### end
            # loss_dis_struct = self.get_rep_pro_loss(vis_rep_s, predicate_proto_vis_s, rel_labels)
            # add_losses.update({"loss_dis_struct": loss_dis_struct * 0.2})
