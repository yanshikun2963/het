...
from hetsgg.modeling.roi_heads.relation_head.utils_motifs import encode_box_info
from hetsgg.modeling.roi_heads.relation_head.model_msg_passing import PairwiseFeatureExtractor,PairwiseFeatureExtractor2
...
class HetSGG(torch.nn.Module):
    def __init__(self, cfg, in_channels):
        ...
        self.pairwise_feature_extractor2 = PairwiseFeatureExtractor2(cfg, in_channels)
        ...
        self.rel_devec = nn.Sequential(
            nn.Linear(2048, self.dim*2),
            nn.ReLU(True),
            nn.Linear(self.dim*2, self.dim),
            nn.ReLU(True) 
        )
        self.obj_devec = nn.Sequential(
            nn.Linear(512, self.dim*2),
            nn.ReLU(True),
            nn.Linear(self.dim*2, self.dim),
            nn.ReLU(True) 
        )
        ...
        self.rmp2 = RMPLayer(self.dim, self.dim, self.dim, self.dim, self.num_bases, self.n_reltypes, cfg=cfg)
    ...
    def forward(self,      
        ...
        # rel_indx, edge_type, edge_type_inv, triples_tensor = self._get_map_idxs2()
        # rel_indx = rel_indx.cuda()
        # ns_f, es_f = self.pairwise_feature_extractor2(triples_tensor)
        # struct_edge_type_list = [edge_type, edge_type_inv]
        # ns_f = self.obj_devec(ns_f).cuda()
        # es_f = self.rel_devec(es_f).cuda()
        # for _ in range(self.feat_update_step):
        #     ns_f,es_f = self.rmp2(rel_indx, ns_f,es_f, struct_edge_type_list)
        #     ns_f = F.elu(ns_f)
        #     es_f = F.elu(es_f)

        # rel_type = triples_tensor[:,1]
        # final_rel = []
        # for i in range(51):
        #     idx = torch.where(rel_type == i)[0]
        #     t = es_f[idx,:]
        #     s = torch.mean(t,dim=0).unsqueeze(0)
        #     final_rel.append(s)
        # final_rel = cat(final_rel,dim=0)
        # return nf, ef, ns_f, es_f,final_rel
    ...
    def _get_map_idxs2(self):


        """
            从三元组文件中读取数据，生成关系索引和边类型。

            参数:
                triples_file (str): 包含三元组的 .txt 文件路径。
                is_training (bool): 是否处于训练模式。

            返回:
                rel_inds (torch.Tensor): 关系索引，形状为 [2, num_relations]。
                edge_types (torch.Tensor): 边类型，形状为 [num_relations]。
                edge_types_inv (torch.Tensor): 反向边类型，形状为 [num_relations]。
        """
        # 初始化变量
        # rel_inds = []  # 存储关系索引
        # edge_types = []  # 存储边类型
        # edge_types_inv = []  # 存储反向边类型
        triples = []
        triples_file = "/home/yj/zgw/het2/proto/triples_id.txt"
        with open(triples_file, 'r') as f:
            lines = f.readlines()

        # 解析三元组
        for line in lines:
            head, relation, tail = line.strip().split()  # 分割三元组
            head = int(head)  # 转换为整数
            relation = int(relation)  # 转换为整数
            tail = int(tail)  # 转换为整数
            triples.append([head, relation, tail])  # 加入列表

        # 将三元组列表转换为 PyTorch 张量
        triples_tensor = torch.tensor(triples, dtype=torch.long)


        head = triples_tensor[:,0].unsqueeze(dim=1)
        relation = triples_tensor[:,1].T
        tail = triples_tensor[:,2].unsqueeze(dim=1)
        rel_indx = torch.cat((head,tail), dim=1)
        proposal_category = self.vg_map_arr[torch.arange(151).long(),1]

        edge_type = torch.LongTensor([self.obj2rtype[(s.item(),d.item())] for (s, d) in proposal_category[rel_indx]]).cuda()  #正向边的类型
        edge_type_inv = torch.LongTensor([self.obj2rtype[(d.item(), s.item())] for (s, d) in proposal_category[rel_indx]]).cuda() # 反向边的类型 
        

        

        return rel_indx.T, edge_type, edge_type_inv, triples_tensor
