...
# 验证测试
# router = UniformMoERouter2()
# test_data = torch.randn(1324, 2048)  # 极小批量测试
# expert_outputs, loss, indices = router(test_data)
# # print(f"强制分配验证: {[len(idx) for idx in indices]}")
# a = cat([expert_outputs[0],expert_outputs[1],expert_outputs[2]],dim=0)
# idx = cat([indices[0],indices[1],indices[2]])
# sort = torch.argsort(idx)
# o = a[sort]

# print(torch.equal(o,test_data))
# print(expert_outputs[0].shape[0], expert_outputs[1].shape[0],expert_outputs[2].shape[0])
# reconstructed ,_ = reconstruct_data(expert_outputs,indices,original_shape=test_data.shape)
# print(torch.allclose(test_data, reconstructed))
...
class UniformMoERouter3_(nn.Module):
    def __init__(self, input_dim=2048, num_experts=3, balance_weight=0.1,
    d_rel = 512, d_graph = 512, ):
        super().__init__()
        # self.gate = nn.Sequential(nn.Linear(input_dim, 1024),
        ...
        #     torch.randn(num_experts, 1, 2048)
        # )
        # nn.init.normal_(self.expert, std=0.02)
