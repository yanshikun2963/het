...
from hetsgg.data.datasets.visual_genome import resampling_dict_generation, get_VG_statistics, \
    apply_resampling
from hetsgg.structures.bounding_box import BoxList
from hetsgg.structures.boxlist_ops import split_boxlist, cat_boxlist
from hetsgg.utils.comm import get_rank, synchronize
...
class OIDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, img_dir, ann_file, cate_info_file, transforms=None,
        ...
        self.vg_cat_dict = json.load(open(cfg.DATA_DIR + '/Category_Type_Info.json', 'r'))
        (self.ind_to_classes,
        ...
        logger = logging.getLogger("hetsgg.dataset")
        self.logger = logger
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING and self.split == 'train':
            ...
            assert self.resampling_method in ['bilvl', 'lvis', 'mylvl']
    ...
    def get_statistics(self):
        ...
        rel_counter = Counter()
        entity_type_predicate_distribution = []
        for i in tqdm(self.idx_list):
            ...
            if self.filter_duplicate_rels:
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.DATA_RESAMPLING_METHOD == 'mylvl':
                    relation = [(k[0], k[1], v[0])
                                for k, v in all_rel_sets.items()]
                relation = np.array(relation, dtype=np.int32)
            gt_label= self.get_groundtruth(i).extra_fields['labels'].numpy()
            if self.repeat_dict is not None:
            ...
            type_list = []
            for s, o, i in relation:
                type_list.append([self.vg_cat_dict['labelidx_to_catidx'][str(gt_label[s])], self.vg_cat_dict['labelidx_to_catidx'][str(gt_label[o])], i])
 
            for i in relation[:, -1]:
                if i > 0:
                    rel_counter[i] += 1
            entity_type_predicate_distribution.append(np.array(type_list))
            
        path = os.path.join(cfg.OUTPUT_DIR, "category_counter.pkl")
        with open(path, 'wb') as f:
            pickle.dump(entity_type_predicate_distribution, f)
